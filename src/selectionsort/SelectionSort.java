...
public class SelectionSort {
    public static void sort(int[] values){
        ...
        while(lastUnsorted > 0){
            ...
            System.out.print("After " + (values.length - lastUnsorted) + " iteration(s): ");
        ...
        // Minimum value swap
        lastUnsorted = values.length - 1; // reset the end of the unsorted section
        while(lastUnsorted > 0){
            // find the minimal unsorted element...
            int minIndex = 0;             // this will be its index
            int minValue = values[0];     // and this will be its value
            for(int i = 1; i <= lastUnsorted; i++)
                if(values[i] < minValue){ // new minimal value
                    minIndex = i;
                    minValue = values[i];
                }
            // then swap it with the last one, and add it to the sorted section
            values[minIndex] = values[lastUnsorted];
            values[lastUnsorted] = minValue;
            lastUnsorted--;
    ...
    public static void main(String[] args) {
        ...
        int numValues = 10; // Adjust based on your testing requirements
